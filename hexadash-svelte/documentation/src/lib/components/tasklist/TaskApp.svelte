<script>
	...
	const taskDataSorted = taskData.slice(0, taskLimit);
	let tasks = createTasks(taskDataSorted);
	...
	let favorite = $tasks.filter((item) => item.favorite);
	let completed = $tasks.filter((item) => item.completed);
</script>
						...
						{#if actionBtn}
							<div class="task-list-btn px-25 mb-25">
								<button class="color-primary"
									><i class="uil uil-plus me-1 fs-16" /> Add New Task</button
								>
							</div>
						{/if}
					{:else}
