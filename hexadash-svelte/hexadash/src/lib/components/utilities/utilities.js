...
const customTooltips = function(context) {
  ...
  if (!this.chart?.canvas.closest('.parentContainer').contains(tooltipEl)) {
    ...
    this.chart?.canvas.closest('.parentContainer').appendChild(tooltipEl);
  }
...
const inlineSvg = function(){
  document.querySelectorAll('img.svg').forEach(function (e) {
    ...
    fetch(imgURL).then(function (response) {
        ...
        svg && e.parentNode?.replaceChild(svg, e);
    });
  });
}

export { chartLinearGradient, customTooltips, inlineSvg, textRefactor };
