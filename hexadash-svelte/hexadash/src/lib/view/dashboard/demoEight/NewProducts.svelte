...
// @ts-nocheck
	...
	import newProducts from '@data/table-data.json';
	import NewProducts from '@components/tables/NewProductsOld.svelte';
	const products = newProducts.newProductOld;
	let period = 'today';
	let currentTab = 'today';
	let dataPeriod = products[period];
	function changeData(data) {
		dataPeriod = products[data];
		currentTab = data;
	}
</script>

<Card class="border-0 px-25 new-products h-100">
