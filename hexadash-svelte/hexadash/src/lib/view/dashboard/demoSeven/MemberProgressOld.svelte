...
// @ts-nocheck
	...
	import MembersProgress from '@components/tables/MembersProgressOld.svelte';
	import progressData from '@data/table-data.json';
	const membersProgress = progressData.memberProgressDataOld;
	let period = 'today';
	let currentTab = 'today';
	let dataPeriod = membersProgress[period];
	function changeData(data) {
		dataPeriod = membersProgress[data];
		currentTab = data;
	}
</script>

<Card class="border-0 px-25 pb-15 h-100">
